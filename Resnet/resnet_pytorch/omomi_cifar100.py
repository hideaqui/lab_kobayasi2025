...
def get_dataloader_cifar100(root="data_cifar100", batch_size=1024, shuffle=True):
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4865, 0.4409), (0.2673, 0.2564, 0.2762))  # CIFAR-100 の平均・標準偏差
    ])
...
def train(total_epoch: int = 200, batch_size: int = 1024):
    ...
    # **CIFAR-100 データローダーの取得**
    train_dataloader, test_dataloader = get_dataloader_cifar100(root="data_cifar100", batch_size=batch_size, shuffle=True)
    ...
    # **モデルの取得（num_classes=100 に変更）**
    model = get_resnet(pretrained=False, num_classes=100, disable_bn=False).to(device)
    ...
    output_dir = f"/Users/hide/卒業研究/resnet_pytorch/output_cifar100/{timestamp}"
