...
from model.resnet50 import get_resnet50  # ✅ ResNet50 を使用
...
def train(total_epoch: int = 200):
    ...
    model = get_resnet50(pretrained=False).to(device)  # ✅ ResNet50 に変更

    # ✅ `conv1` のカーネルサイズを 3×3, stride=1 に変更 (MNIST 向け)
    model.conv1 = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1, bias=False)

    # ✅ BatchNorm の `momentum=0.01` に変更 (統計が安定する)
    for module in model.modules():
        if isinstance(module, nn.BatchNorm2d):
            module.momentum = 0.01

    # ✅ `AdamW(lr=0.001)` に変更 (SGD より安定)
    optimizer = optim.AdamW(model.parameters(), lr=0.001)

    # ✅ 学習率スケジューラを `step_size=10` に変更 (減衰を遅くする)
    ...
    def hook(module, input, output):
        """最後のミニバッチの `layer4` の出力を保存 (64, 2048, 1, 1)"""
        nonlocal last_activation
        last_activation = output.cpu().detach().numpy().reshape(-1, 2048, 1, 1)  # ✅ ResNet50 用に修正

    model.layer4.register_forward_hook(hook)

    # ✅ **フォルダ名を `output_resnet50` に変更**
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = f"/Users/hide/卒業研究/resnet_pytorch/output_resnet50/{timestamp}"
