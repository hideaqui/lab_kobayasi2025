...
# **パーシステンスの平均値を格納**
h0_persistence_means = []
h1_persistence_means = []
valid_epochs = []

print("🔹 Extracting PH persistence means...")
for epoch in tqdm(epoch_range, desc="Processing epochs", unit="epoch"):
    ...
    # **PH を計算（Ripserの警告修正: distance_matrix=Trueを追加）**
    diagrams = ripser(distance_matrix, maxdim=1, metric="precomputed", distance_matrix=True)["dgms"]
    ...
    h0_persistence_means.append(h0_mean)
    h1_persistence_means.append(h1_mean)
    valid_epochs.append(epoch)

# **可視化**
plt.figure(figsize=(8, 5))
plt.plot(valid_epochs, h0_persistence_means, label="H0 のパーシステンスライフ", marker="o", linestyle="-")
plt.plot(valid_epochs, h1_persistence_means, label="H1 のパーシステンスライフ", marker="s", linestyle="--")
