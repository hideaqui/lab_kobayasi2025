...
# ★ 新しい `output_dir`
output_dir = "/Users/hide/卒業研究/resnet_pytorch/output_6000/20250125_030030"
accuracy_csv_path = os.path.join(output_dir, "epoch_accuracies.csv")

num_epochs = 10  # 1 ~ 10

# epoch_accuracies.csv から generalization_gap を取得（エポック1-10）
accuracy_df = pd.read_csv(accuracy_csv_path)
generalization_gaps = accuracy_df["generalization_gap"].values[1:num_epochs + 1]  # 1~10
...
for epoch in range(1, num_epochs + 1):  # 1 ~ 10
    ...
    for dim in range(3):
        ...
        if len(finite_bars) == 0:
            ...
            lam_mean = np.mean(lifetimes)
            lam_var  = np.var(lifetimes)
            mu_mean  = np.mean(midpoints)
            mu_var   = np.var(midpoints)
...
valid_epochs = np.array(valid_epochs)  # エポック 1-10 のリスト
...
y = generalization_gaps[:len(valid_epochs)]

# generalization_gap も標準化
y_mean, y_std = np.mean(y), np.std(y)
y_scaled = (y - y_mean) / y_std

# 線形回帰モデル
model = LinearRegression()
model.fit(all_features_scaled, y_scaled)

# 回帰結果
coeffs = model.coef_
intercept = model.intercept_
predicted_gaps_scaled = model.predict(all_features_scaled)

# 逆変換
predicted_gaps = predicted_gaps_scaled * y_std + y_mean

r2_score = model.score(all_features_scaled, y_scaled)
mse = mean_squared_error(y_scaled, predicted_gaps_scaled)
rmse = np.sqrt(mse)
...
for i, col_name in enumerate(columns):
    result_df[col_name] = all_features_scaled[:, i]

# CSV保存
result_path = os.path.join(output_dir, "regression_results_dimwise_1-10_normalized.csv")
result_df.to_csv(result_path, index=False)
print(f"✅ 正規化後の回帰結果を保存しました: {result_path}")