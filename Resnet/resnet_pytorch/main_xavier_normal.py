    ...
    for epoch in range(total_epoch):
        ...
        for batch_idx, (images, labels) in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{total_epoch}")):
            ...
            with torch.no_grad():
                ...
                if batch_idx % 20 == 0:  # 20バッチごとに保存
                    np.savez(
                        os.path.join(output_dir, f"epoch_{epoch+1}_batch_{batch_idx}_weights.npz"),
                        **batch_weights
                    )
        scheduler.step()
...
def save_epoch_accuracies():
    accuracy_log, test_accuracy_log, generalization_gap, output_dir = train(total_epoch=20, mode="xavier_normal", seed=seed)
