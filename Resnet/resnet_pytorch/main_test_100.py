...
def train(total_epoch: int = 100):
    ...
    output_dir = f"/Users/hide/卒業研究/resnet_pytorch/output/100epoch_{timestamp}"
    ...
    generalization_gap.append(train_acc_0 - test_acc_0)  # ✅ 修正：絶対値を使わない

    print(f"Epoch 0: Train Acc = {train_acc_0:.4f}, Test Acc = {test_acc_0:.4f}, Gap = {generalization_gap[0]:.4f}")

    # **エポックごとの学習**
    for epoch in range(1, total_epoch + 1):  # ✅ 修正: 1から100まで
        model.train()
        train_loss = 0.0
        train_correct = 0
        train_total = 0
        ...
        generalization_gap.append(train_acc - test_acc)  # ✅ 修正：絶対値を使わない
